        ...
        if uploaded_file2 is not None:
            ...
            if Selected_y.shape[0] <= 1:
                ...
                for column in columns:
                        value = st.sidebar.number_input(column, 0.00, 1000.00, df2[column].mean(),format="%.3f") #int(df2[column].mean()))
            ...
            #Target = st.sidebar.number_input(list(Selected_y)[0], 0.00, 1000.00, 0.00,format="%.3f")
                st.sidebar.write('**Target Value :**')
                N_sample = 0
                Target = st.sidebar.number_input(Selected_y[0], 0.00, 1000.00, 0.00,format="%.3f")
                ...
                for column in columns:
                    ...
                    max1 = round(float(df[column].max()),5)
                    min1 = round(float(df[column].min()),5)
                
                    rag1 = round(min1+((max1-min1)*0.1),5)
                    rag2 = round(min1+((max1-min1)*0.9),5)
        ...
        #parameter_n_neighbors = st.sidebar.slider('Number of neighbers', 2, 10, (1,6), 2)  
            ...
            #Target = st.sidebar.number_input(list(Selected_y)[0], 0.00, 1000.00, 0.00,format="%.3f")
                ...
                for i in range(sample_target):
                    Target.append(st.sidebar.number_input(Selected_y[i], 0.00, 1000.00, 0.00,format="%.3f"))
                ...
                #para4 = pd.DataFrame(para4)
                    ...
                    for i in range(1,df[Selected_y].shape[1]+1):
                        ...
                        sns.lineplot(x=para4.index,y=Target[i-1], color='red')
